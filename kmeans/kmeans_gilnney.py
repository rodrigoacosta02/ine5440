...
class KMeans():
    ...
    def _reevaluate_centers(self):
        ...
        # self.mu = newmu
        self.mu = spatial.KDTree(newmu)
 
    def _has_converged(self):
        oldmu = self.oldmu.data
        mu = self.mu.data
        K = len(oldmu)

        muset = set([tuple(a) for a in mu]) 
        return (len(muset) == K and muset == set([tuple(a) for a in oldmu]))
    ...
    def find_centers(self, method='random'):
        ...
        self.oldmu = spatial.KDTree(random.sample(X, K))
        if method != '++':
            # Initialize to K random centers
            # self.mu = random.sample(X, K)
            self.mu = spatial.KDTree(random.sample(X, K))
